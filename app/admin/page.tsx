...
export default function AdminDashboard() {
  ...
  // STATE UNTUK MENANGKAP SIARAN CCTV
  ...
  // PENERIMA SIARAN SUPABASE (WEBSOCKET)
  useEffect(() => {
    if (!isAuthenticated) return;
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <input type="password" placeholder="ENTER SECURE PIN" value={pinInput} onChange={(e) => setPinInput(e.target.value)}
                className={`w-full bg-black/40 border-2 rounded-xl p-4 text-center text-white text-xl tracking-[0.4em] font-mono focus:outline-none transition-all placeholder-white/30 ${loginError ? 'border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'border-white/20 focus:border-[#009CB4]'}`} autoFocus />
        ...
        <div className={`min-h-screen bg-[#111827] py-12 flex flex-col items-center gap-12 print:bg-white print:py-0 print:gap-0 print:block transition-all duration-300 ${showSignPanel ? 'ml-80' : ''}`}>
          ...
          <div className="w-[210mm] h-[296mm] bg-white shadow-[0_0_40px_rgba(0,0,0,0.5)] p-[10mm] print:shadow-none print:w-full print:h-[296mm] print:p-[10mm] relative flex flex-col overflow-hidden">
            ...
            <div className="border-2 border-black flex-1 flex flex-col">
              ...
              <div className="p-4 border-b border-black min-h-45 relative">
          ...
          <div className="w-[210mm] h-[296mm] bg-white shadow-[0_0_40px_rgba(0,0,0,0.5)] p-[10mm] print:shadow-none print:w-full print:h-[296mm] print:p-[10mm] relative flex flex-col overflow-hidden page-break">
                  ...
                  <div className="flex items-center gap-2">
                    {[1, 2, 3].map(num => (
                      <div key={num} className="flex items-center gap-1 border border-black px-1">
                        <div className={`w-3 h-3 border border-black flex items-center justify-center text-[10px]`}>{resExamResult?.exam_no === num ? 'X' : ''}</div>
                        <span>{num}</span>
                      </div>
                    ))}
               ...
               {[0, 25, 50, 75].map((startIdx) => (
                  <div key={startIdx} className="flex-1 border-r last:border-r-0 border-gray-400">
                     ...
                     {Array.from({ length: 25 }).map((_, i) => {
                        ...
                        return (
                          <div key={no} className="grid grid-cols-[20px_1fr_1fr_1fr_1fr] text-[10px] border-b border-gray-300 h-5 items-center">
            ...
            <div className="mt-4 text-[10px]">
               ...
               <div className="flex border border-black h-40">
                 ...
                 <div className="w-1/4 border-r border-black flex flex-col">
                   <div className="flex-1 border-b border-gray-400 p-2 flex items-center">
    ...
    <div className="min-h-screen bg-[#F4F6F9] font-sans pb-20">
      ...
      <div className="bg-[#002561] text-white pt-10 pb-24 px-6 md:px-12 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative z-10">
          <div className="flex items-center gap-5">
            <div className="bg-white p-3 rounded-2xl shadow-lg"><img src="/logo.png" alt="Logo" className="h-10 w-auto object-contain" /></div>
            <div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={handleLogout} className="px-5 py-2.5 bg-red-600/80 hover:bg-red-600 text-white rounded-xl font-bold text-sm shadow-[0_0_15px_rgba(220,38,38,0.4)] transition-all border border-red-500">
              <span>🚪</span> Logout
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 -mt-12 space-y-10 relative z-20">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col">
          <div className="p-6 md:p-8 border-b border-gray-100 bg-white flex justify-between items-center gap-4">
            <h2 className="text-xl font-black text-[#002561] tracking-wider uppercase flex items-center gap-3">
              <span className="p-2 bg-blue-50 text-blue-600 rounded-lg text-lg">🔑</span> Active Access Codes
            </h2>
          ...
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50/50 text-[#002561] border-b border-gray-100 text-xs uppercase tracking-wider font-bold">
                <tr>
              ...
              <tbody className="divide-y divide-gray-50">
                {tokens.length === 0 ? (
                  <tr><td colSpan={5} className="p-8 text-center text-gray-400 italic">No access codes generated yet.</td></tr>
                ) : tokens.map((token) => (
                  <tr key={token.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="p-5">
                      <span className="px-3 py-1 bg-gray-100 text-gray-600 rounded-md font-bold text-xs border border-gray-200">
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => { navigator.clipboard.writeText(token.access_code); alert(`Tersalin!`) }} className="p-2 text-gray-400 hover:text-[#009CB4] hover:bg-[#009CB4]/10 rounded-lg transition-all opacity-0 group-hover:opacity-100">
                          📋
                        </button>
                      </div>
                    </td>
                    <td className="p-5 pr-8 text-center">
                      ...
                      <button onClick={() => handleDeleteToken(token.id, token.access_code)} className="p-1.5 text-red-300 hover:text-red-500 transition-all ml-2">
                        🗑️
                      </button>
                    </td>
                  </tr>
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col">
          <div className="p-6 md:p-8 border-b border-gray-100 bg-white">
            <h2 className="text-xl font-black text-[#002561] tracking-wider uppercase flex items-center gap-3">
              <span className="p-2 bg-blue-50 text-blue-600 rounded-lg text-lg">🧑‍✈️</span> Live Participants & Proctoring CCTV
            </h2>
          </div>
          ...
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-50">
                {sessions.length === 0 ? (
                  <tr><td colSpan={6} className="p-8 text-center text-gray-400 italic">No participants found.</td></tr>
                  ...
                  // AMBIL DARI STATE LIVE WEB SOCKET
                    ...
                    <tr key={session.id} className="hover:bg-blue-50/30 transition-colors">
                      <td className="p-5 pl-8 align-middle relative group">
                        {session.status === 'COMPLETED' ? (
                           (candPhoto ? <img src={candPhoto} onClick={() => setSelectedPhoto(candPhoto)} className="w-20 h-15 rounded-xl object-cover border-2 border-gray-200 opacity-50 cursor-pointer" /> : <div className="w-20 h-15 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-400 font-bold">?</div>)
                        ) : (
                           (liveSnapshot ? (
                              <div className="relative cursor-pointer hover:scale-105 transition-transform" onClick={() => setSelectedLiveCam(session.id)}>
                                <img src={liveSnapshot} className="w-20 h-15 rounded-xl object-cover border-2 border-red-400 shadow-[0_0_10px_rgba(239,68,68,0.3)] bg-black" alt="Live Cam" />
                                <div className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border-2 border-white shadow-md"></div>
                                <span className="absolute -bottom-2 -left-2 bg-red-600 text-white text-[7px] font-black px-1.5 py-0.5 rounded tracking-widest shadow-md">LIVE</span>
                              </div>
                           ) : (
                              <div className="w-20 h-15 rounded-xl bg-gray-900 border-2 border-gray-700 flex flex-col items-center justify-center text-gray-500 text-center p-1">
                      ...
                      <td className="p-5 align-middle">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[11px] font-black text-gray-600 tracking-widest uppercase mt-0.5">
                            {candUnit}
                          </span>
                        </div>
                      </td>
                            ...
                            <>
                              <div className="flex items-center gap-2">
                                <span className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 text-amber-700 rounded-md border border-amber-200 text-[10px] font-bold uppercase tracking-widest animate-pulse">
                                  <div className="w-2 h-2 rounded-full bg-amber-500"></div> Progress
                                </span>
                      ...
                      <td className="p-5 align-middle">
                        {session.status === 'COMPLETED' ? (
                          <div className="flex items-center gap-2">
                            <div className={`px-3 py-1 text-[11px] font-black tracking-widest uppercase rounded-md border shadow-sm ${isPassed ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-red-500 text-white border-red-600'}`}>
                              {isPassed ? 'PASSED' : 'FAILED'}
                            </div>
                            {isPassed ? (
                              <button onClick={() => handleAdjustResult(session.id, false)} title="Force Fail" className="p-1.5 text-gray-400 hover:text-red-600 transition-all">❌</button>
                            ) : (
                              <button onClick={() => handleAdjustResult(session.id, true)} title="Force Pass" className="p-1.5 text-gray-400 hover:text-emerald-600 transition-all">✓</button>
                            )}
                          </div>
                        ) : (
                        ...
                        <div className="flex items-center justify-center gap-2">
                          {session.status === 'COMPLETED' && (
                            <>
                              ...
                              <button onClick={() => handleSendEmail(session)} className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg border transition-all ${session.email_sent ? 'bg-gray-100 text-gray-400' : 'bg-white text-[#009CB4] border-[#009CB4] hover:bg-[#009CB4]/10'}`}>
                                📧 {session.email_sent ? 'Sent' : 'Email'}
                              </button>
                            </>
                          )}
                          <button onClick={() => resetParticipant(session.id, candName)} title="Delete" className="p-1.5 text-gray-300 hover:text-red-500 transition-all">
                            🗑️
                          </button>
                        </div>
                      </td>
